...
<aside class="fixed top-0 left-0 h-full w-[270px] bg-white text-gray-800 border-r border-gray-200/80 shadow-sm z-50 
    ...
    <nav class="flex-1 px-4 py-6 overflow-y-auto">
        ...
        <a href="{{ url('/') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-lg {{ Request::is('/') ? 'bg-blue-800 text-white shadow-md shadow-blue-900/30' : 'text-gray-600 hover:bg-gray-50 hover:text-blue-800' }} mb-1 transition" title="Dashboard">
        ...
        <!-- MANAJEMEN SECTION -->
        <p class="text-[10px] uppercase tracking-wider text-gray-400 font-semibold px-3 mt-8 mb-4 sidebar-text">Manajemen</p>

        <a href="{{ url('/users') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-lg {{ Request::is('users*') ? 'bg-blue-800 text-white shadow-md shadow-blue-900/30' : 'text-gray-600 hover:bg-gray-50 hover:text-blue-800' }} mb-1 transition" title="Manajemen User">
        ...
        <div class="mb-1">
            <button type="button" 
                ...
                <i class="fas fa-arrow-down text-[10px] transition-transform duration-300 sidebar-text {{ Request::is('settings*') ? 'rotate-180' : '' }}" id="settingSubmenuArrow"></i>
            </button>

            <!-- Nested Submenu Children with Slide-Down Animation & Real Arrow Indicators (fa-arrow-right) -->
            <div id="settingSubmenuList" class="{{ Request::is('settings*') ? 'open-submenu' : 'hidden-submenu' }} pl-9 pr-2 space-y-1">
                <a href="{{ url('/settings/profile') }}" class="flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-semibold {{ Request::is('settings/profile') ? 'bg-blue-100 text-blue-800 font-bold' : 'text-gray-600 hover:bg-gray-50 hover:text-blue-800' }} transition">
                    <i class="fas fa-arrow-right text-[10px] opacity-70"></i>
                    <span class="sidebar-text">Profil Pengguna</span>
                </a>
                <a href="{{ url('/settings/app') }}" class="flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-semibold {{ (Request::is('settings/app') || Request::is('settings')) ? 'bg-blue-100 text-blue-800 font-bold' : 'text-gray-600 hover:bg-gray-50 hover:text-blue-800' }} transition">
                    <i class="fas fa-arrow-right text-[10px] opacity-70"></i>
                    <span class="sidebar-text">Pengaturan Sistem</span>
                </a>
                <a href="{{ url('/settings/logs') }}" class="flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-semibold {{ Request::is('settings/logs') ? 'bg-blue-100 text-blue-800 font-bold' : 'text-gray-600 hover:bg-gray-50 hover:text-blue-800' }} transition">
                    <i class="fas fa-arrow-right text-[10px] opacity-70"></i>
        ...
        <!-- LAPORAN SECTION -->
        <p class="text-[10px] uppercase tracking-wider text-gray-400 font-semibold px-3 mt-8 mb-4 sidebar-text">Laporan</p>
        
        <a href="#" class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-gray-600 hover:bg-gray-50 hover:text-blue-800 transition mb-1" title="Export Excel">
        ...
        <a href="#" class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-gray-600 hover:bg-gray-50 hover:text-blue-800 transition mb-1" title="Export PDF">
    ...
    <div class="px-4 py-5 border-t border-gray-100 flex-shrink-0">
        <div class="flex items-center gap-3 user-footer-container">
            <div class="w-9 h-9 rounded-full bg-blue-900/10 text-blue-800 border border-blue-900/20 flex items-center justify-center font-semibold text-sm flex-shrink-0">
                AD
            </div>
