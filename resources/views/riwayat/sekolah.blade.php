        ...
        <div class="relative z-10 flex flex-wrap items-center justify-between gap-4">
            <div class="max-w-2xl space-y-1">
                <div class="flex items-center gap-2 text-blue-200 text-xs mb-1">
                    ...
                    <a href="{{ route('sekolah.show', $sekolah->id) }}" class="hover:underline opacity-80">{{ $sekolah->nama_sekolah }}</a>
                    <i class="fas fa-chevron-right text-[10px]"></i>
                ...
                <p class="text-blue-100 text-xs opacity-90 mt-1">
                    <i class="fas fa-school mr-1"></i>{{ $sekolah->nama_sekolah }}
                    &nbsp;|&nbsp; NPSN: <span class="font-mono">{{ $sekolah->npsn }}</span>
                    &nbsp;|&nbsp; Kec. {{ $sekolah->kecamatan }}
                </p>
            </div>
            <div class="flex flex-wrap items-center gap-2">
                <a href="{{ route('sekolah.show', $sekolah->id) }}" class="bg-white/15 hover:bg-white/25 border border-white/20 text-white text-xs font-bold px-4 py-2.5 rounded-lg flex items-center gap-2 transition">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div class="stat-card bg-white rounded-xl p-4 border border-gray-100 shadow-xl shadow-gray-200/50 flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs font-bold text-gray-900 mt-0.5">{{ $sekolah->created_at?->format('d M Y, H:i') }} WIB</p>
                </div>
            </div>
            <div class="stat-card bg-white rounded-xl p-4 border border-gray-100 shadow-xl shadow-gray-200/50 flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs font-bold text-gray-900 mt-0.5">{{ $sekolah->updated_at?->format('d M Y, H:i') }} WIB</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200/80 shadow-md overflow-hidden">
                                                ...
                                                @foreach($log->changes as $field => $change)
                                                    <tr class="hover:bg-emerald-50/20">
                                                        <td class="px-4 py-2.5 font-semibold text-gray-700">
                                                            {{ \App\Models\ActivityLog::sekolahFieldLabel($field) }}
                                                        </td>
            ...
            @else
                <div class="px-6 py-16 text-center">
                    ...
                    <p class="text-xs text-gray-400 mt-1">Riwayat akan otomatis tercatat saat data sekolah ini diedit atau diperbarui.</p>
                </div>
            @endif
        </div>
    </div>
