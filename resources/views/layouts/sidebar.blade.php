...
<aside class="fixed top-0 left-0 h-full w-[270px] bg-white text-gray-800 border-r border-gray-200/80 shadow-sm z-50
    ...
    <nav class="flex-1 px-4 py-6 overflow-y-auto">
        ...
        <div class="mt-4 pt-4 border-t border-gray-100 space-y-1">
            <p class="text-[10px] uppercase tracking-wider text-gray-400 font-semibold px-3 mb-2 sidebar-text">Publik
            </p>

            @if(Auth::user() && method_exists(Auth::user(), 'isAdminDinas') && Auth::user()->isAdminDinas())
                <a href="{{ route('pengumuman.index') }}"
                    class="flex items-center gap-3 px-4 py-2.5 rounded-lg {{ Request::is('admin/pengumuman*') ? 'bg-blue-800 text-white shadow-md shadow-blue-900/30' : 'text-gray-600 hover:bg-gray-50 hover:text-blue-800' }} mb-1 transition"
        ...
        <div class="mt-4 pt-4 border-t border-gray-100 space-y-1">
            <p class="text-[10px] uppercase tracking-wider text-gray-400 font-semibold px-3 mb-2 sidebar-text">
                Pengaturan</p>
            ...
            <div class="relative">
                <button type="button" id="settingMenuBtn"
                    ...
                    <i class="fas fa-arrow-right text-[10px] transition-transform duration-300 {{ Request::is('settings*') ? 'rotate-90' : '' }} sidebar-text"
                        id="settingArrow"></i>
                </button>

                <!-- Accordion Submenu Links (With Smooth Slide-down Animation) -->
                <div id="settingSubmenu"
                    class="sidebar-accordion-submenu {{ Request::is('settings*') ? 'accordion-expanded' : '' }} pl-9 pr-2 py-2 space-y-1">
    ...
    <!-- User Section -->
    <div class="p-4 border-t border-gray-100 flex-shrink-0 user-footer-container">
        <div class="flex items-center gap-3 px-2">
            <img src="{{ Auth::user()->avatar ?? 'https://ui-avatars.com/api/?name=' . urlencode(Auth::user()->name ?? 'Admin Dinas') . '&background=1e40af&color=fff' }}"
                alt="User Avatar" class="w-9 h-9 rounded-full object-cover border border-gray-200">
            <div class="flex-1 min-w-0 sidebar-text">
                <p class="font-bold text-xs text-gray-900 truncate">{{ Auth::user()->name ?? 'Administrator Dinas' }}
                </p>
                <p class="text-[10px] text-gray-400 truncate">{{ Auth::user()->email ?? 'admin@dinas.go.id' }}</p>
            </div>
            <a href="{{ route('logout') }}"
                class="text-gray-400 hover:text-red-600 p-1.5 rounded-lg hover:bg-red-50 transition sidebar-text"
                title="Keluar">
