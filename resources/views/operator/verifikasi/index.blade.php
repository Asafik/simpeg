...
@section('content')
    ...
    <div class="px-6 md:px-8 pb-8 flex-1 space-y-6 -mt-8 relative z-20">
        ...
        <!-- Pagination (Matches Custom SIMPEG-SP UI Styling) -->
        @if(isset($pegawais) && method_exists($pegawais, 'hasPages') && $pegawais->hasPages())
            <div class="mt-6 p-4 bg-white rounded-xl border border-gray-100 flex flex-col lg:flex-row items-center justify-between gap-3.5 shadow-sm text-center lg:text-left">
                <span class="text-xs text-gray-500 font-medium">
                    Halaman <span class="font-bold text-gray-800">{{ $pegawais->currentPage() }}</span> dari <span class="font-bold text-gray-800">{{ $pegawais->lastPage() }}</span> (Menampilkan {{ $pegawais->firstItem() }} - {{ $pegawais->lastItem() }} dari {{ $pegawais->total() }} Data Verifikasi)
                </span>
