...
<header id="landingNavbar" class="w-full fixed top-0 left-0 right-0 z-50 transition-all duration-300 bg-transparent py-5">
    <div class="w-full px-6 md:px-12 flex items-center justify-between">
        ...
        <nav class="hidden md:flex items-center gap-8 text-xs font-semibold text-blue-100/90">
            <a href="{{ url('/') }}" class="{{ (Request::is('/') || Request::is('landing')) && !Request::is('landing/*') ? 'text-white font-bold border-b-2 border-white pb-1' : 'hover:text-white transition' }}">
                Beranda
            </a>
            <a href="{{ url('/landing/statistik') }}" class="{{ Request::is('landing/statistik') ? 'text-white font-bold border-b-2 border-white pb-1' : 'hover:text-white transition' }}">
                Statistik Data
            </a>
            <a href="{{ url('/landing/layanan') }}" class="{{ Request::is('landing/layanan') ? 'text-white font-bold border-b-2 border-white pb-1' : 'hover:text-white transition' }}">
                Layanan & Keunggulan
            </a>
            <a href="{{ url('/landing/cek-ptk') }}" class="{{ Request::is('landing/cek-ptk') ? 'text-white font-bold border-b-2 border-white pb-1' : 'hover:text-white transition' }}">
                Cek Data PTK
            </a>
            <a href="{{ url('/landing/pengumuman') }}" class="{{ Request::is('landing/pengumuman') ? 'text-white font-bold border-b-2 border-white pb-1' : 'hover:text-white transition' }}">
                Pengumuman
            </a>
        </nav>
        ...
        <div class="flex items-center gap-3">
            ...
            @else
                ...
                <a href="{{ url('/landing/cek-ptk') }}" class="hidden sm:flex bg-white/15 backdrop-blur-md hover:bg-white/25 border border-white/20 text-white font-bold text-xs px-5 py-2.5 rounded-xl shadow-md transition items-center gap-1.5">
