...
<header id="landingNavbar" class="w-full fixed top-0 left-0 right-0 z-50 transition-all duration-300 bg-transparent py-5">
    <div class="w-full px-6 md:px-12 flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
            ...
            <button id="mobileMenuToggle" class="xl:hidden text-white hover:text-blue-200 focus:outline-none p-2 rounded-lg bg-white/10 border border-white/20 transition">
...
<div id="mobileSidebar" class="fixed inset-0 z-50 translate-x-full transition-transform duration-300 ease-in-out pointer-events-none">
    <!-- Backdrop -->
    <div id="mobileSidebarBackdrop" class="absolute inset-0 bg-black/50 opacity-0 transition-opacity duration-300 ease-in-out pointer-events-none"></div>

    <!-- Drawer Content -->
    <div class="absolute right-0 top-0 bottom-0 w-72 bg-blue-950 text-white shadow-2xl flex flex-col justify-between p-6 pointer-events-auto border-l border-blue-800/40">
        <div class="space-y-8">
            <!-- Header -->
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                    ...
                    <div>
                        ...
                        <p class="text-[9px] uppercase tracking-wider text-blue-200/80 font-semibold">Dinas Pendidikan</p>
                    </div>
                </div>
                <button id="mobileSidebarClose" class="text-white hover:text-blue-200 focus:outline-none p-1.5 rounded-lg bg-white/10 border border-white/20 transition">
            ...
            <nav class="flex flex-col gap-4 text-sm font-semibold text-blue-100/90">
                <a href="{{ url('/') }}" class="flex items-center gap-3 py-2 px-3 rounded-xl hover:bg-white/10 hover:text-white transition {{ (Request::is('/') || Request::is('landing')) && !Request::is('statistik*') && !Request::is('layanan*') && !Request::is('cek-ptk*') && !Request::is('pengumuman*') ? 'bg-white/15 text-white font-bold' : '' }}">
                    <i class="fas fa-house text-xs w-5"></i>
                    <span>Beranda</span>
                </a>
                <a href="{{ url('/statistik') }}" class="flex items-center gap-3 py-2 px-3 rounded-xl hover:bg-white/10 hover:text-white transition {{ Request::is('statistik*') || Request::is('landing/statistik*') ? 'bg-white/15 text-white font-bold' : '' }}">
                    <i class="fas fa-chart-pie text-xs w-5"></i>
                    <span>Statistik Data</span>
                </a>
                <a href="{{ url('/layanan') }}" class="flex items-center gap-3 py-2 px-3 rounded-xl hover:bg-white/10 hover:text-white transition {{ Request::is('layanan*') || Request::is('landing/layanan*') ? 'bg-white/15 text-white font-bold' : '' }}">
                    <i class="fas fa-layer-group text-xs w-5"></i>
                    <span>Layanan & Keunggulan</span>
                </a>
                <a href="{{ url('/cek-ptk') }}" class="flex items-center gap-3 py-2 px-3 rounded-xl hover:bg-white/10 hover:text-white transition {{ Request::is('cek-ptk*') || Request::is('landing/cek-ptk*') ? 'bg-white/15 text-white font-bold' : '' }}">
                    <i class="fas fa-magnifying-glass text-xs w-5"></i>
                    <span>Cek Data PTK</span>
                </a>
                <a href="{{ url('/pengumuman') }}" class="flex items-center gap-3 py-2 px-3 rounded-xl hover:bg-white/10 hover:text-white transition {{ Request::is('pengumuman*') || Request::is('landing/pengumuman*') ? 'bg-white/15 text-white font-bold' : '' }}">
        ...
        <div class="space-y-3 pt-6 border-t border-blue-900">
            @auth
                <a href="{{ route('dashboard') }}" class="w-full bg-blue-800 hover:bg-blue-900 border border-blue-400/30 text-white font-extrabold text-xs py-3 rounded-xl shadow-lg transition flex items-center justify-center gap-2">
                    <i class="fas fa-chart-pie text-xs"></i>
                    <span>Buka Dashboard</span>
                </a>
            @else
                <a href="{{ url('/login') }}" class="w-full bg-blue-800 hover:bg-blue-900 border border-blue-400/30 text-white font-extrabold text-xs py-3 rounded-xl shadow-lg transition flex items-center justify-center gap-2">
                    <i class="fas fa-right-to-bracket text-xs"></i>
                    <span>Login Portal</span>
                </a>
                <a href="{{ url('/cek-ptk') }}" class="w-full bg-white/10 hover:bg-white/20 border border-white/20 text-white font-bold text-xs py-3 rounded-xl shadow-md transition flex items-center justify-center gap-2">
